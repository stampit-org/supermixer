...
test('uniquely merging two objects', (t) => {
  ...
  t.equal(result.last, 1, 'should merge non existing properties one level deep');
  t.end();
});
...
test('uniquely merging two objects with function props', (t) => {
  ...
  const result = merge({ deep1: original, deep2: { deeper: original } }, o1, o2, { last: noop });
  ...
  t.equal(result.last, noop, 'should merge non existing functions one level deep');
  t.end();
});
